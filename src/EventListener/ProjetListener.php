...
class ProjetListener implements EventSubscriberInterface
{
    ...
    public function createFolders(BeforeEntityPersistedEvent $event)
    {
        ...
        if ($entity instanceof Projet) {
            ...
            // Si le dossier de production existe et que la ref interne du client est renseignée
            if ($filesystem->exists(Constants::PROJET_PATH) && trim($event->getEntityInstance()->getRefInterne()) != '') {
                try {
